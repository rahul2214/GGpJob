...
async function testNewOnboardingFlow() {
  ...
  const signupRes = await fetch('http://localhost:9500/api/auth/signup', {
    method: 'POST',
  ...
  const validateRes = await fetch('http://localhost:9500/api/referral/validate', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const onboardRes = await fetch(`http://localhost:9500/api/users/${userBefore.uuid}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      name: 'Test New Flow User',
