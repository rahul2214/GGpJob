...
async function testFullUserPayload() {
  console.log("Simulating full POST /api/jobs with user payload...");
  ...
  if (insertError) {
    console.error("Full payload job insert FAILED:", insertError);
  } else {
    console.log("FULL USER PAYLOAD JOB CREATION SUCCESSFUL! Job ID:", newJob.id, "UUID:", newJob.uuid);
    await supabaseAdmin.from('jobs').delete().eq('id', newJob.id);
    console.log("Cleaned up test job.");
  }
}

testFullUserPayload().catch(console.error);
