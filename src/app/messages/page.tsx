...
export default function MessagesPage() {
  ...
  const fetchChats = async () => {
    ...
    try {
      ...
      if (user.role === "Recruiter") {
        url = `/api/applications?recruiterId=${user.uuid}&requesterId=${user.uuid}`;
  ...
  const filteredChats = chats
    .filter((chat) => {
      ...
      const nameMatch = user?.role === "Recruiter" 
        ? chat.applicantName.toLowerCase().includes(query)
        : (chat.posterName || "Recruiter").toLowerCase().includes(query);

      const headlineMatch = user?.role === "Recruiter"
      ...
      const skillsMatch = user?.role === "Recruiter"
  ...
  // Authorize only seeker & recruiter
  if (user && !["Job Seeker", "Recruiter"].includes(user.role)) {
    return (
          ...
          <CardHeader className="flex flex-row items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700 font-medium">
                Unified messaging is currently optimized for Job Seekers and Recruiters.
              </p>
            </div>
          </CardHeader>
          <CardContent className="text-sm text-slate-600">
            You are logged in as a <strong>{user.role}</strong>. Admin dashboards have independent management views.
          </CardContent>
          <CardFooter>
        ...
        <div className="border-b border-slate-100 pb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2.5 text-slate-600 max-w-2xl text-sm leading-relaxed">
              {isRecruiter 
                ? "Interact directly with candidates who applied to your postings."
                : "Chat directly with recruiters regarding your applications."}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            ...
            <div className="relative w-full sm:w-72">
              ...
              <Input
                type="text"
                placeholder={isRecruiter ? "Search candidates, jobs, skills..." : "Search companies..."}
                value={searchQuery}
          ...
          <Card className="border-dashed py-16 text-center rounded-[2rem] bg-white">
            <CardContent className="flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-xs text-slate-500 max-w-sm font-medium leading-relaxed">
                {chats.length === 0 
                  ? (isRecruiter 
                      ? "You don't have any active candidate chats yet." 
                      : "You don't have any active chats yet. Apply to jobs to start chatting.")
                  : "No matches found. Try resetting filters or search term."}
              </p>
              {chats.length === 0 && !isRecruiter && (
                <Button onClick={() => router.push("/jobs")} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl mt-2 text-xs">
                  Browse Jobs
                ...
                >
                  ...
                  >
                    ...
                    <CardHeader className="pb-3 pt-6 px-6">
                      <div className="flex items-start gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-500 to-violet-600 text-white font-bold text-sm shadow-md flex items-center justify-center shrink-0">
                          {getInitials(isRecruiter ? chat.applicantName : (chat.posterName || "Recruiter"))}
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5 justify-between">
                            <h4 className="text-sm font-black text-slate-900 truncate">
                              {isRecruiter ? chat.applicantName : (chat.posterName || "Recruiter")}
                            </h4>
                            {chat.unreadChatCount > 0 && (
                    ...
                    <CardContent className="flex-1 px-6 pb-4 flex flex-col gap-4">
                      ...
                      {/* Skills Tags (Recruiter View Only) */}
                      {isRecruiter && chat.applicantSkills && (
                    ...
                    <CardFooter className="pt-3 border-t border-slate-100 bg-slate-50/40 p-4 flex gap-2">
                      {isRecruiter && chat.applicantResumeUrl && (
                        <Button 
                          asChild 
                          variant="outline" 
