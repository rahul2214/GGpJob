...
export function ApplyButton({ job, variant = 'default', isApplied: propIsApplied = false, onSuccess }: ApplyButtonProps) {
    ...
    const isJobOwner = (user && job) && (user.role === 'Recruiter' || user.role === 'Admin') && 
                       (job.recruiterId === user.uuid || job.recruiterPk === user.id);
    ...
    return (
        <>
            ...
            >
                ...
                <span>
                    {isApplied ? 'Applied' : (buttonLoading ? (isCheckingAts ? 'Checking Match...' : 'Applying...') : 'Apply Now')}
                </span>
            </Button>
