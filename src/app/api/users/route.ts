    ...
    if (uid) {
        ...
        if (jobseeker) {
            ...
            const user: any = {
                ...
                countryId: jobseeker.cities?.states_provinces?.countries?.id || jobseeker.current_country_id || null,
                stateId: jobseeker.cities?.states_provinces?.id || jobseeker.current_state_province_id || null,
                ...
                workplaceTypeId: jobseeker.workplace_type_id || jobseeker.workplace_types?.id || undefined,
                workplaceType: jobseeker.workplace_types?.name || (jobseeker.workplace_type_id === 1 ? 'Remote' : jobseeker.workplace_type_id === 2 ? 'On-site' : jobseeker.workplace_type_id === 3 ? 'Hybrid' : jobseeker.workplace_type_id === 4 ? 'Flexible / Any' : null) || jobseeker.workplace_type || '',
                ...
                noticePeriod: jobseeker.notice_periods?.name || jobseeker.notice_period || '',
                noticePeriodId: jobseeker.notice_period_id || jobseeker.notice_periods?.id || undefined,
                hasUsedAtsChecker: jobseeker.has_used_ats_checker ?? jobseeker.metadata?.has_used_ats_checker ?? false,
                has_used_ats_checker: jobseeker.has_used_ats_checker ?? jobseeker.metadata?.has_used_ats_checker ?? false,
                hasSeenReferralPrompt: jobseeker.has_seen_referral_prompt ?? jobseeker.metadata?.hasSeenReferralPrompt ?? false,
                has_seen_referral_prompt: jobseeker.has_seen_referral_prompt ?? jobseeker.metadata?.hasSeenReferralPrompt ?? false,
                referralStepDismissed: jobseeker.referral_step_dismissed ?? jobseeker.metadata?.referralStepDismissed ?? false,
                referral_step_dismissed: jobseeker.referral_step_dismissed ?? jobseeker.metadata?.referralStepDismissed ?? false,
                hasUsedResumeBuilder: jobseeker.has_used_resume_builder ?? jobseeker.metadata?.has_used_resume_builder ?? false,
                has_used_resume_builder: jobseeker.has_used_resume_builder ?? jobseeker.metadata?.has_used_resume_builder ?? false,
                referralRewarded: jobseeker.referral_rewarded ?? jobseeker.metadata?.referral_rewarded ?? false,
                referral_rewarded: jobseeker.referral_rewarded ?? jobseeker.metadata?.referral_rewarded ?? false,
                referralRewardedAt: jobseeker.referral_rewarded_at || jobseeker.metadata?.referral_rewarded_at || null,
                referral_rewarded_at: jobseeker.referral_rewarded_at || jobseeker.metadata?.referral_rewarded_at || null,
