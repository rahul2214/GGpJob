...
import { LoaderCircle, ShieldCheck, Zap, Lock, CheckCircle2, Star, Crown, Tag, Globe } from "lucide-react";
...
export default function JobSeekerPlansPage() {
  ...
  useEffect(() => {
    router.replace('/jobseeker/credits');
  }, [router]);
    ...
    <div className="min-h-screen bg-[#f8fafc] py-20 px-6 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {plans.map((plan, idx) => {
            const convertedPrice = convertPrice(plan.price, currency, exchangeRates);
            const formattedPrice = formatPrice(convertedPrice, currency);

            let convertedOriginal = 0;
            let formattedOriginal = "";
            if (plan.originalPrice) {
              convertedOriginal = convertPrice(plan.originalPrice, currency, exchangeRates);
              formattedOriginal = formatPrice(convertedOriginal, currency);
            }
                    ...
                    <div className="flex items-center justify-between gap-2.5 mb-2 w-full">
                      {plan.originalPrice && plan.price > 0 ? (
                        <div className="flex flex-col">
                          <div className="flex items-center gap-2">
                            <span className="text-white/60 font-medium text-lg line-through decoration-white/40">{formattedOriginal}</span>
               ...
               <div className="p-4 bg-white border-b flex items-center justify-between border-slate-100">
                  <div className="flex items-center gap-3">
                     <div className={cn("w-10 h-10 rounded-2xl flex items-center justify-center border", 
                        selectedPlan.color === 'emerald' ? "bg-emerald-50 border-emerald-100 text-emerald-600" : 
                        selectedPlan.color === 'sky' ? "bg-sky-50 border-sky-100 text-sky-600" : "bg-indigo-50 border-indigo-100 text-indigo-600")}>
