...
const formSchema = z.object({
  ...
  message: "Personal email domains (Gmail, Yahoo, Outlook, etc.) are not allowed for corporate signup. Please use your official company email address.",
  path: ["email"],
});
...
export default function CompanySignupPage() {
  ...
  const onSubmit = async (data: SignupFormValues) => {
    setEmailError(null);
    try {
      const fullPhone = data.phone ? `${countryCode}${data.phone.replace(/^\+/, '')}` : undefined;
        ...
        <div className="relative z-10 space-y-8">
          ...
          >
            <p className="text-white/90 text-sm italic leading-relaxed mb-3">
              "We filled 3 senior engineering roles in under 10 days using direct hiring listings. Incredible platform!"
            </p>
