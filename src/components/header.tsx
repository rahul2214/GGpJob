...
export default function Header() {
  ...
  useEffect(() => {
    if (isClient && user && user.role === 'Recruiter') {
  ...
  const renderMobileLeftButton = () => {
    const isRecruiterOrEmployee = user?.role === 'Recruiter';
                            ...
                            {isClient && !userLoading && user && !isPlanSelectionPage && (
                             ...
                             {isClient && !userLoading && user?.role === 'Job Seeker' && !isPlanSelectionPage && (
                              ...
                              {isClient && !userLoading && !user && (
                               <>
                                 ...
                                 <SheetClose asChild>
                                     <Link href="/ats-score" className={cn("flex items-center justify-between text-muted-foreground hover:text-foreground w-full", pathname === '/ats-score' && "text-foreground font-bold")}>
                                         <span className="flex items-center gap-3">
                                            <Sparkles className="h-5 w-5" />
                                            ATS Checker
                                         </span>
                                         <span className="text-[9px] font-bold bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 px-1.5 py-0.5 rounded-full uppercase tracking-wider">
                                           Free
                                         </span>
                                     </Link>
                                 </SheetClose>
                             ...
                             {isClient && !userLoading && user && user.role === 'Recruiter' && !isPlanSelectionPage && (
                                  (!(user.planType === 'basic' && jobCount >= 1)) && (
                                     <SheetClose asChild>
                                        <Link href="/jobs/post" className="flex items-center gap-3 text-muted-foreground hover:text-foreground">
      ...
      <nav className="ml-6 hidden md:flex items-center gap-6 text-sm font-medium">
        {isClient && !userLoading && user && (user.role === 'Admin' || user.role === 'Super Admin') ? (
          ...
          <>
            ...
            {isClient && !userLoading && user && user.role === 'Recruiter' && !isPlanSelectionPage && (
              (!(user.planType === 'basic' && jobCount >= 1)) && (
                <Link href="/jobs/post" className={`transition-colors hover:text-foreground ${pathname === '/jobs/post' ? "text-foreground font-bold border-b-2 border-primary pb-1" : "text-foreground/60"}`}>
                    Post Job
                </Link>
              )
            )}
            ...
            {isClient && !userLoading && !user && (
              <>
                ...
                <Link href="/ats-score" className={`transition-colors hover:text-foreground flex items-center gap-1.5 ${pathname === "/ats-score" ? "text-foreground font-bold border-b-2 border-primary pb-1" : "text-foreground/60"}`}>
                ...
                {(user.role === 'Job Seeker' || user.role === 'Recruiter') && (
                 ...
                 <div className="hidden md:flex items-center gap-2">
                    ...
                    <DropdownMenuContent align="end">
                        ...
                        {['Job Seeker', 'Recruiter'].includes(user.role) && (
                            <DropdownMenuItem asChild>
                                <Link href="/feedback">
