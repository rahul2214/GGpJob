...
export default function CareerAssistant() {
  ...
  useEffect(() => {
    ...
    let welcomeMsg = "Hi 👋 I am your AI Career Assistant. I can help you recommend matching jobs, optimize your resume/ATS score, or run mock interviews. What would you like to do?";
    let welcomeSuggestions = ["🔍 Recommend Jobs", "📄 Improve Resume & ATS", "🎯 Mock Interview", "⚡ Direct Apply"];

    if (user?.role === "Recruiter") {
      welcomeMsg = "Hi 👋 I am your AI Career Assistant. I can help you source matching candidates, review applicant resumes/ATS profiles, or optimize your job posts. What would you like to do?";
      welcomeSuggestions = ["👥 Match Candidates", "📝 Optimize Job Post", "📋 Screen Applicants"];
    } else if (user?.role === "Admin") {
      welcomeMsg = "Hi 👋 I am your AI Career Assistant. I can help you monitor system analytics and manage portal configurations. How can I help you today?";
      welcomeSuggestions = ["📊 System Analytics"];
    }
  ...
  const parseInlineMarkdown = (text: string) => {
    // Render markdown links: [text](url) - ignore javascript: schemes
    ...
    return lines.map((line, idx) => {
      ...
      if (cleanLine.startsWith("- ") || cleanLine.startsWith("* ")) {
        ...
        return (
          <li 
            key={idx} 
            className="ml-4 list-disc text-slate-700 my-1 leading-relaxed text-xs break-words whitespace-pre-wrap" 
      ...
      if (/^\d+\.\s/.test(cleanLine)) {
        ...
        return (
          <li 
            key={idx} 
            className="ml-4 list-decimal text-slate-700 my-1 leading-relaxed text-xs break-words whitespace-pre-wrap" 
            dangerouslySetInnerHTML={{ __html: parseInlineMarkdown(content) }} 
          />
        );
      }
      ...
      if (cleanLine.startsWith("# ")) {
        return (
          <h1 
            key={idx} 
            className="text-sm font-bold text-slate-900 mt-3 mb-1 break-words whitespace-pre-wrap" 
      ...
      if (cleanLine.startsWith("## ")) {
        return (
          <h2 
            key={idx} 
            className="text-xs font-bold text-slate-800 mt-2 mb-1 break-words whitespace-pre-wrap" 
      ...
      if (cleanLine.startsWith("### ")) {
        return (
          <h3 
            key={idx} 
            className="text-xs font-semibold text-slate-700 mt-1.5 mb-0.5 break-words whitespace-pre-wrap" 
      ...
      return (
        <p 
          key={idx} 
          className="text-slate-700 my-1 leading-relaxed text-xs break-words whitespace-pre-wrap" 
  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-[9999] flex flex-col items-end max-w-[calc(100vw-32px)] sm:max-w-none">
      {/* Floating Chat Window */}
      {isOpen && (
        <div className="mb-4 w-[calc(100vw-32px)] sm:w-[380px] h-[550px] max-h-[80vh] flex flex-col bg-white/95 backdrop-blur-md border border-slate-200 shadow-2xl rounded-3xl overflow-hidden transition-all duration-300 transform scale-100 translate-y-0 origin-bottom-right">
          {/* Header */}
                  ...
                  return (
                    <div 
                      ...
                      className={cn(
                        ...
                        isUser ? "ml-auto flex-row-reverse pl-6 pr-1" : "mr-auto pr-6 pl-1"
                      )}
                    >
                      {/* Avatar */}
                      <div className={cn(
                      ...
                      <div className="flex flex-col gap-2 min-w-0">
                        <div className={cn(
                          "px-4 py-3 rounded-2xl shadow-sm text-xs border break-words whitespace-pre-wrap",
                          isUser
              ...
              <Input
                placeholder={
                  user?.role === "Recruiter"
                    ...
                    : user?.role === "Admin"
                    ? "Ask about system metrics, verification, audits..."
                    : "Ask about jobs, resume help..."
                }
                value={inputValue}
